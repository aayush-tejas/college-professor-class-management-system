...
function showImportStudentsModal() {
    ...
    // Bind button events
    document.getElementById('previewBtn').addEventListener('click', previewExcelData);
    document.getElementById('importBtn').addEventListener('click', importStudentData);
}
...
function handleFileSelection(event) {
    ...
    if (file) {
        ...
        if (!isValidType) {
            app.showAlert('Please select a valid Excel (.xlsx, .xls) or CSV file.', 'danger');
            event.target.value = '';
            return;
        }
        ...
        if (file.size > 10 * 1024 * 1024) {
            app.showAlert('File size must be less than 10MB.', 'danger');
            event.target.value = '';
            return;
        }
...
async function previewExcelData() {
    ...
    if (!file) {
        app.showAlert('Please select a file first.', 'warning');
        return;
    }
    ...
    try {
        ...
        app.showAlert('Failed to preview file: ' + error.message, 'danger');
    } finally {
        app.hideLoading();
    }
}
...
function displayPreviewData(data) {
    const { headers, previewRows, mappedFields, totalRows, requiredFieldsFound } = data;
    ...
    document.getElementById('previewBody').innerHTML = rowsHtml;
    
    // Display field mappings
    const mappingHtml = Object.entries(mappedFields).map(([field, columnIndex]) => {
        const fieldLabels = {
        ...
        const isRequired = ['studentId', 'firstName', 'lastName', 'email'].includes(field);
        const badgeClass = isRequired ? 'bg-primary' : 'bg-secondary';
        
        return `
            <div class="col-md-6 mb-2">
                <div class="d-flex justify-content-between align-items-center">
                    <span class="badge ${badgeClass}">${fieldLabels[field] || field}</span>
                    <span class="text-muted small">→ Column ${columnIndex + 1}: "${headers[columnIndex]}"</span>
                </div>
            </div>
        `;
    }).join('');
    ...
    if (requiredFieldsFound) {
        validationHtml = `
            <div class="alert alert-success">
                ...
                All required fields found! Ready to import ${totalRows} students.
            </div>
        `;
        document.getElementById('importBtn').classList.remove('d-none');
    } else {
        const requiredFields = ['Student ID', 'First Name', 'Last Name', 'Email'];
        const foundFields = Object.keys(mappedFields).filter(field => 
            ['studentId', 'firstName', 'lastName', 'email'].includes(field)
        );
        const missingFields = requiredFields.filter((_, index) => 
            !foundFields.includes(['studentId', 'firstName', 'lastName', 'email'][index])
        );
...
async function importStudentData() {
    ...
    try {
        ...
        // Refresh students list if we're on the students page
        if (app.loadStudents && typeof app.loadStudents === 'function') {
            app.loadStudents();
        }
        
    } catch (error) {
        app.showAlert('Failed to import students: ' + error.message, 'danger');
    } finally {
...
function displayImportResults(results) {
    const { totalRows, successCount, errorCount, duplicateCount, errors, duplicates } = results;
    
    let resultHtml = `
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0"><i class="fas fa-chart-bar me-1"></i>Import Results</h6>
            </div>
            <div class="card-body">
    ...
    if (errors.length > 0) {
        resultHtml += `
            <div class="alert alert-danger">
                <h6><i class="fas fa-exclamation-triangle me-1"></i>Errors:</h6>
                <ul class="mb-0 small">
    ...
    if (duplicates.length > 0) {
        resultHtml += `
            <div class="alert alert-warning">
                <h6><i class="fas fa-info-circle me-1"></i>Duplicates Skipped:</h6>
                <ul class="mb-0 small">
                    ${duplicates.slice(0, 5).map(duplicate => `<li>${duplicate}</li>`).join('')}
                    ${duplicates.length > 5 ? `<li><em>... and ${duplicates.length - 5} more duplicates</em></li>` : ''}
                </ul>
            </div>
        `;
    }
    ...
    if (successCount > 0) {
        app.showAlert(`Successfully imported ${successCount} students!`, 'success');
    }
}
